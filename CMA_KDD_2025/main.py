...
def parse_args():
    ...
    parser.add_argument('--output', type=str, default='/home/shannon_research/share/jhq/meta_output/', 
    ...
    parser.add_argument('--adapt', action='store_true', default=True, help='adapt or not.')
    parser.add_argument('--future_incontext', action='store_true', default=False, help='adapt incontext or not.')
    parser.add_argument('--pretrained', action='store_true', default=False, help='not after pretrained or after pretrained.')
...
def main():
    ...
    if args.train:
        ...
        if args.adapt == 1:
            ...
            samples, *_ = trainer.restore_adapt(dataloader, [args.pred_len, dataset.var_num], \
                                                coef, stepsize, sampling_steps,args.future_incontext)
        elif args.jump == 1 :
            samples, *_ = trainer.restore_jump(dataloader, [args.pred_len, dataset.var_num], coef, stepsize, sampling_steps)
        else:
            samples, *_ = trainer.restore(dataloader, [args.pred_len, dataset.var_num], coef, stepsize, sampling_steps)
