...
class CustomDataset(Dataset):
    def __init__(
        ...
        self.dir = os.path.join('/home/shannon_research/share/jhq/meta_output/', 'samples')
        ...
        if self.adapt!=0:
            ...
            self.real_pred_len = self.pred_len
        self.adapt_h = adapt_h
        self.window, self.period = window, period
        self.window_adapt = self.feat_len + self.real_pred_len
        self.adapt_his_len = adapt_his_len
        # if self.adapt_h:
        ...
        self.sample_num_train = max(self.train_data.shape[0] - self.window_adapt + 1,0)
        self.sample_num_val = max(self.val_data.shape[0] - self.window_adapt + 1,0)
        self.sample_num_test = max(self.test_data.shape[0] - self.window_adapt + 1,0)
        
        
        #train, inference = self.__getsamples(self.data, proportion, seed)
        train,train_mark,train_incontext,train_incontext_mark = \
            self.__get_train_samples(self.train_data,self.train_data_stamp)
        inference,inference_mark,inference_incontext,inference_incontext_mark = \
    ...
    def __get_train_samples(self, data,data_mark):
        ...
        # train_incontxt_data = np.zeros((train_data.shape[0]-self.choice_number-self.num_step,self.num_step,self.window,self.var_num))
        # train_incontxt_mark = np.zeros((train_data.shape[0]-self.choice_number-self.num_step,self.num_step,self.window,data_mark.shape[1]))
        # for i in range(self.choice_number+self.num_step,train_data.shape[0]):
        #     choice = list(range(i-self.choice_number,i-self.choice_number+self.num_step))
        #     # print(choice)
        #     # exit()
        #     train_incontxt_data[i-self.choice_number-self.num_step,:,:,:] = train_data[choice,:,:]
        #     train_incontxt_mark[i-self.choice_number-self.num_step,:,:,:] = train_mark[choice,:,:]
            
        
        # train_data = train_data[self.choice_number+self.num_step:,:,:]
        # train_mark = train_mark[self.choice_number+self.num_step:,:,:]
    ...
    def __get_test_samples(self, data,data_mark):
        ...
        x_mark = np.zeros((self.sample_num_test, self.window, data_mark.shape[1]))
    ...
    def __get_val_samples(self, data,data_mark):
        ...
        x = np.zeros((self.sample_num_val, self.window, self.var_num))
        x_mark = np.zeros((self.sample_num_val, self.window, data_mark.shape[1]))
        
        if self.adapt==0:
            adapt_num = 0
        else:
            adapt_num = self.adapt - 1
        x_incontex = np.zeros((self.sample_num_val, self.window+(adapt_num)*self.pred_len, self.var_num))
        x_mark_incontext = np.zeros((self.sample_num_val, self.window+(adapt_num)*self.pred_len, data_mark.shape[1]))
        
        for i in range(self.sample_num_val):
            start = i
            end = i + self.window
            x[i, :, :] = data[start:end, :]
            x_mark[i, :, :] = data_mark[start:end, :]
            ...
            if self.adapt!=0:
                end_incontext = i + self.window + (adapt_num)*self.pred_len
        ...
        if self.adapt_h:
            x_incontex = x_incontex[self.adapt_his_len-self.feat_len:,:,:]
            x_mark_incontext = x_mark_incontext[self.adapt_his_len-self.feat_len:,:,:]
        # 读取数据集的时候,真实值已经恢复了原本的scaler
        if self.save2npy:
        ...
        if self.adapt!=0:
            ...
            np.save(os.path.join(self.dir, f"{self.name}_ground_truth_{self.window}_test_adapt.npy"), self.unnormalize_in(x_incontex,self.window+(self.pred_len)*(self.adapt-1)))
        ...
        return test_data,test_mark,test_incontxt_data,test_incontxt_mark
    ...
    def __getitem__(self, ind):
        if self.period == 'test':
            ...
            return torch.from_numpy(x).float(), torch.from_numpy(m),torch.from_numpy(x_mark),\
                torch.from_numpy(x_incontext).float(),torch.from_numpy(x_incontext_mark).float()
        x = self.samples[ind, :, :]  # (seq_length, feat_dim) array
        x_mark = self.samples_mark[ind,:,:]
        # x_incontext = self.samples_incontext[ind,:,:,:]
        # x_incontext_mark = self.samples_incontext_mark[ind,:,:,:]
