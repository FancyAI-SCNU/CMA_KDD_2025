...
class CustomDataset(Dataset):
    def __init__(
        ...
        self.rawdata, self.scaler, self.data_stamp = self.read_data(
            data_root, self.name)
        file = "ecl_scaler.sav"
        pickle.dump(self.scaler, open(file, 'wb'))
        ...
        # print(period,missing_ratio,predict_length)
        # exit()
        if period == 'test':
    ...
    def __get_train_samples(self, data, data_mark):
        ...
        train_incontxt_data = np.zeros(
            (self.sample_num_train, self.num_step, self.window, self.var_num))
        train_incontxt_mark = np.zeros(
            (self.sample_num_train, self.num_step, self.window, data_mark.shape[1]))
    ...
    def __get_val_samples(self, data, data_mark):
        ...
        # print(data_mark.shape,self.real_pred_len,self.feat_len)
        ...
        if self.adapt != 0:
            ...
            np.save(os.path.join(self.dir, f"{self.name}_ground_truth_{self.window}_test_adapt.npy"), self.unnormalize_in(
                x_incontex, self.real_pred_len+self.feat_len))

            if self.auto_norm:
                np.save(os.path.join(
                    self.dir, f"{self.name}_norm_truth_{self.window}_test_adapt.npy"), unnormalize_to_zero_to_one(x_incontex))

            else:
                np.save(os.path.join(
                    self.dir, f"{self.name}_norm_truth_{self.window}_test_adapt.npy"), x_incontex)

        test_incontxt_data = np.zeros(
            (self.sample_num_test, self.num_step, self.window, self.var_num))
        test_incontxt_mark = np.zeros(
            (self.sample_num_test, self.num_step, self.window, data_mark.shape[1]))
        for i in range(self.sample_num_test):
            choice = list(range(i, i+self.adapt_num_step *
                          self.his_interval, self.his_interval))
    ...
    def read_data(filepath, name=''):
        ...
        scaler = scaler.fit(data[:train_len, :])
