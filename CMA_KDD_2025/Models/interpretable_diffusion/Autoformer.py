...
class EncoderBlock(nn.Module):
    ...
    def __init__(self,
        ...
        attention = AutoCorrelationLayer(
                        AutoCorrelation(False,attention_dropout=attn_pdrop),
                        d_model= n_embd
                        )
        self.attention = attention
        self.conv1 = nn.Conv1d(in_channels=n_embd, out_channels=d_ff, kernel_size=1, bias=False)
        self.conv2 = nn.Conv1d(in_channels=d_ff, out_channels=n_embd, kernel_size=1, bias=False)
        moving_avg = 25
        self.decomp1 = series_decomp(moving_avg)
        self.decomp2 = series_decomp(moving_avg)
        
        self.dropout = nn.Dropout(resid_pdrop)
        self.activation = F.relu if activate == "relu" else F.gelu
        self.ln1 = AdaLayerNorm(n_embd)

    def forward(self, x, timestep, mask=None, label_emb=None):
        print(label_emb,"111")
        x= self.ln1(x, timestep, label_emb)
        new_x, attn,_ = self.attention(
            x, x, x,
            attn_mask=mask
        )
...
class Transformer(nn.Module):
    ...
    def forward(self, input, t, padding_masks=None, return_res=False):
        # input torch.Size([128, 24, 7])
        emb = self.emb(input)
        # emb torch.Size([128, 24, 64])
